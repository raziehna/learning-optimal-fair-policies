...
setwd("/Users/danielmalinsky/Dropbox/fair policy learning/submission_ICML/compas/")
...
#source("compas_Vsearch.R")
#source("compas_Vsearch_unfair.R")
#source("compas_Gestimation.R")
#source("compas_Gestimation_unfair.R") 
...
## Utility
Y_recid = dat$recid
sink("compas_experiment6.txt")

#for(i in c(14.0,12.0,10.0,8.0,6.0,4.0,2.0,1.0,-1.0,-2.0)){
#for(i in c(3.0,2.5,2.4,2.3,2.2,2.1,2.0)){
#for(i in c(2.52,2.5,2.48,2.46,2.44,2.42,2.4,2.38,2.36,2.34)){
...
C1 = -abs( rnorm(n,1.5,1) )
theta = i
cat("theta : ", theta, "\n")
C2 = -abs( rnorm(n,1.5*theta,1) )
C3 = abs( rnorm(n,1.5,1) )
...
# (eff_SY = pse_SY_SM(dat, beta_M, inter_M_S, inter_Y_S))
...
funcSY = pse_SY_SM 
funcSA = pse_SA_SM
# beta_start = c(beta_M, beta_Y)
beta_start = NULL

#result_f = optimize_nloptr(dat, fmla_Y, fmla_M, funcSY, funcSA, inter_M_S, inter_Y_S, 
#                           var, tau_cont_u, tau_cont_l, tau_bin_u, tau_bin_l, beta_start)
...
plot(sort(theta.vec),ratios_f,type = "l", lwd = 2, lty = "dashed", xaxt = "n", xlab = "Utility Parameter", ylab = "Incarceration Rate", 
     main = "Incarceration Rate as a Function of Utility Parameter in the COMPAS Data")
...
plot(sort(theta.vec),fairY.vec,type="l",lwd=2,lty="dashed",xaxt = "n", xaxt = "n", xlab = "Utility Parameter", ylab = "Expected Utility", 
     main = "Relative Utility of Policy as a Function of Utility Parameter in the COMPAS Data")
axis(1, at=1:4, labels=c(1,2,3,4))
