...
# --------------------------------------
optimize_nloptr <- function(dat, fmla_M, funcSY, funcSB, funcSA, inter_M_S, 
  ...
  eval_g_ineq <- function(beta){
    ...
    # eff_SB - tau_u< 0, tau_l - eff_SB < 0
    eff_SB = funcSB(dat, beta_S, beta_M, inter_M_S)
    eval_SB =  c(eff_SB - tau_bin_u, tau_bin_l - eff_SB)
    ...
    eval_g = c(eval_SY, eval_SB, eval_SA)
    return(eval_g)
  }
